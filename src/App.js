...
class App extends React.Component {
  ...
  handleRegionChange = e => {
    ...
    let regionHasntChanged =
      prevSelectedRegion && selected === prevSelectedRegion.label;
  ...
  handleQuestionChange = e => {
    ...
    let clickedQuestion = questionsCopy.filter(
    ...
    let selectedOption = clickedQuestion.options.filter(
      option => option.value === selected
    )[0];
    let increment = selectedOption.result ? 0 : 1;
    let questionsToShow =
      questions.filter(questions => questions.selectedOption).length +
      increment;
    ...
    question.options
      ? question.options.map(option => (
          <span className="radio-wrapper">
            ...
            />
            <label className="btn" htmlFor={question.value}>
              {option.value}
            </label>
          </span>
        ))
  ...
  selectedRegion = () =>
    this.state.regions.filter(region => region.isSelected)[0];
  ...
  render() {
    ...
    let regionField = (
      <div className="form-group">
        ...
        />
        <p class="small">
          Data provided by Hip Hop Caucus'{" "}
          <a href="https://respectmyvote.com" target="_blank">
            Respect My Vote!
          </a>{" "}
          campaign.
        </p>
    ...
    let questionFields = questions
      ? questions.map((question, i) => (
          <div
            className={i < questionsToShow ? "form-group" : "form-group hide"}
          >
            <label>{question.value}</label>
            <p dangerouslySetInnerHTML={{ __html: question.extra }} />
    ...
    let resultWrapper = (
      <div className={`result-wrapper ${resultClass}`} role="alert">
